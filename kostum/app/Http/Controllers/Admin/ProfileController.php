    ...
    public function create()
{
    ...
    // Ambil hanya pengguna yang memiliki peran "User"
    $users = User::whereHas('roles', function ($query) {
        $query->where('title', 'User'); // Mengambil pengguna dengan peran 'User'
    })->get();
    ...
    public function store(StoreProfileRequest $request)
{
    ...
    // Mendapatkan pengguna yang dipilih dari form
    $user = User::find($request->input('user_id'));

    // Pastikan peran 'User' ada di database dan terhubung dengan pengguna
    $role = Role::where('title', 'User')->first();

    if ($user && $role) {
        // Menambahkan peran 'User' ke pengguna
        $user->roles()->attach($role);
    }

    // Menambahkan media jika ada
