            ...
            <table class="table table-bordered table-striped table-hover datatable datatable-Order">
                <thead>
                    <tr>
                        ...
                        <th>{{ trans('cruds.order.fields.profile_id') }}</th>
                        <th>{{ trans('cruds.order.fields.kode_order') }}</th>
                        <th>{{ trans('cruds.order.fields.tanggal_sewa') }}</th>
                        <th>{{ trans('cruds.order.fields.tanggal_dikembalikan') }}</th>
                ...
                <tbody>
                    @foreach($orders as $order)
                        <tr data-entry-id="{{ $order->id }}">
                            <td></td>
                            <td>{{ $order->id ?? '' }}</td>
                            <td>{{ $order->profile->name ?? $order->profile->email ?? '' }}</td> <!-- Display Profile Name -->
                            <td>{{ $order->kode_order ?? '' }}</td>
                            <td>{{ $order->tanggal_sewa ?? '' }}</td>
                            <td>{{ $order->tanggal_dikembalikan ?? '' }}</td>
                            <td>{{ $order->total ?? '' }}</td>
                            <td>{{ $order->status ?? '' }}</td>
                            <td>
                                @can('order_show')
