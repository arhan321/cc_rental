...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.pesananitem.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route('admin.pesananitems.store') }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('obat_id') ? 'is-invalid' : '' }}" name="obat_id" id="obat_id" required>
                    <option value disabled {{ old('obat_id', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                ...
                @if($errors->has('obat_id'))
                    <div class="invalid-feedback">{{ $errors->first('obat_id') }}</div>
                @endif
            ...
            <div class="form-group">
                ...
                @if($errors->has('pesanan_id'))
                    <div class="invalid-feedback">{{ $errors->first('pesanan_id') }}</div>
                @endif
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('qty') ? 'is-invalid' : '' }}" type="number" name="qty" id="qty" value="{{ old('qty', 1) }}" min="1" step="1" required>
                @if($errors->has('qty'))
