...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route('admin.pengirimans.store') }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('pengirim_id') ? 'is-invalid' : '' }}" name="pengirim_id" id="pengirim_id" required>
                    <option value disabled {{ old('pengirim_id', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                    @foreach($pengirims as $id => $pengirim)
                        <option value="{{ $pengirim->id }}" {{ old('pengirim_id') == $pengirim->id ? 'selected' : '' }}>{{ $pengirim->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('pesanan_id') ? 'is-invalid' : '' }}" name="pesanan_id" id="pesanan_id" required>
                    <option value disabled {{ old('pesanan_id', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                    @foreach($pesanans as $id => $pesanan)
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('jarak') ? 'is-invalid' : '' }}" type="number" step="0.01" name="jarak" id="jarak" value="{{ old('jarak') }}" required>
                @if($errors->has('jarak'))
                    <div class="invalid-feedback">
                        {{ $errors->first('jarak') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total') ? 'is-invalid' : '' }}" type="number" step="0.01" name="total" id="total" value="{{ old('total') }}" required>
                @if($errors->has('total'))
                    <div class="invalid-feedback">
                        {{ $errors->first('total') }}
                    </div>
