        ...
        <form method="POST" action="{{ route('admin.pengajuans.update', [$pengajuan->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('profile_id') ? 'is-invalid' : '' }}" name="profile_id" id="profile_id" required>
                    <option value disabled {{ old('profile_id', $pengajuan->profile_id) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                    @foreach($profiles as $profile)
                        <option value="{{ $profile->id }}" {{ old('profile_id', $pengajuan->profile_id) == $profile->id ? 'selected' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nomor_pengajuan') ? 'is-invalid' : '' }}" type="text" name="nomor_pengajuan" id="nomor_pengajuan" value="{{ old('nomor_pengajuan', $pengajuan->nomor_pengajuan) }}" required>
                @if($errors->has('nomor_pengajuan'))
                    <div class="invalid-feedback">{{ $errors->first('nomor_pengajuan') }}</div>
                @endif
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('catatan') ? 'is-invalid' : '' }}" name="catatan" id="catatan">{{ old('catatan', $pengajuan->catatan) }}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('alamat') ? 'is-invalid' : '' }}" name="alamat" id="alamat" required>{{ old('alamat', $pengajuan->alamat) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('jarak') ? 'is-invalid' : '' }}" type="number" name="jarak" id="jarak" value="{{ old('jarak', $pengajuan->jarak) }}" step="0.01" required>
                @if($errors->has('jarak'))
                    <div class="invalid-feedback">{{ $errors->first('jarak') }}</div>
                @endif
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total') ? 'is-invalid' : '' }}" type="number" name="total" id="total" value="{{ old('total', $pengajuan->total) }}" step="0.01" required>
                @if($errors->has('total'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status" required>
                    <option value disabled {{ old('status', $pengajuan->status) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                    @foreach(App\Models\Pengajuan::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $pengajuan->status) == $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
